...
def stimulus_sequence(experiment_timeline, training_timeline, to_html=False):
    ...
    instruction_1 = TextStimulus(
        text="<p>In this experiment, you will see a sequence of <strong>eight items</strong>, which consist of <strong>six letters</strong> out of the whole alphabet and <strong>two digits</strong> out of '1', '2', '3', and '4'.<br>The items will be presented one after another in the middle of your screen.</p>Press SPACE to continue.",
        choices=[" "],
    )

    instruction_2 = TextStimulus(
        text="<p>Your task will be to focus on the two digits and remember them. You will then also be asked to report them by pressing the according keys on your keyboard. The order in which they were presented doesn't matter here – just the two digits themselves.</p>Press SPACE to continue.",
        choices=[" "],
    )

    instruction_3 = TextStimulus(
        text="<p>There will also be some pattern with moving dots displayed in the background, but you don't have to pay attention to that.<br>Try focusing on the sequence, and in particular, the two digits instead.</p>Press SPACE to continue.",
        choices=[" "],
    )

    # training onboarding
    training_boarding = TextStimulus(
        text="<p>Now let's look at some examples.<br>In the following, we have a couple of training rounds for you to get accustomed to the experiment and the task.</p>Press SPACE to start the training.",
        choices=[" "],
    )

    # experiment onboarding
    experiment_boarding = TextStimulus(
        text="<p>Now that you know how the experiment works and what you have to do, let's start with the actual experiment!<br>It will look exactly like the training you just had. Again, try to focus on the sequence and the two digits in the middle of your sceen. Good luck!</p>Press SPACE to start the experiment.",
        choices=[" "],
    )
    ...
    debriefing = TextStimulus(
        text="<p>Congratulations, you finished the experiment!<br>Thank you for participating and for sticking until the end! I hope you had at least some fun during all of it.</p>Press SPACE to continue.",
        choices=[" "],
    )

    feedback = TextSurveyStimulus(
        prompts=[
            "<p>If you have any feedback for us, we would appreciate hearing about it. Did you encounter any issues during the experiment? Do you have any suggestions for improvement? Or do you have any other comments you want to share with us? Let us know here!</p>"
        ]
    )
    ...
    response_1 = TextStimulus(
        text="<p>Use your keyboard to enter a digit which you recall seeing out of<br>'1', '2', '3', '4'</p>If you cannot remember any number, press 'x'.",
        choices=["1", "2", "3", "4", "x"],
        correct_key=correct_choice,
    )
    response_2 = TextStimulus(
        text="<p>Use your keyboard to enter the other digit you recall seeing out of<br>'1', '2', '3', '4'.</p>If you cannot remember another number, press 'x'.",
        choices=["1", "2", "3", "4", "x"],
        correct_key=correct_choice,
    )
